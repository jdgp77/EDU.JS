...
EduInt.Thing.setCustom('ayudante',function(){
	...
	*/
											...
											children: [
												{
													...
													{
														this.EduIntGDInfo.Board.accAddMessage(GDPlataform.myUser,'mensaje',this.code);
													},
												}
											]
										},
	...
	this.getChatId = function(code)
	{
		return this.g('chat').getChatId(code);
	};
...
var GDPlataform = {
	...
	onAuthStateChanged: function(user){
		if(user)
		{
			...
			*/
			...
			GDPlataform.socket.on('getMyFriends',function(arUsersData){
			...
			GDPlataform.socket.emit('conect_user',GDPlataform.myUser);
		}
		else
		{
	...
	initOneTime: function()
	{
		if(GDPlataform.bnInitOneTime)
		{
			...
			//	Socket.io
			GDPlataform.socket = io.connect('http://localhost:3000');
			GDPlataform.socket.on('connect', function() {
				console.info(GDPlataform.socket.id);
				GDPlataform.myUser.socketid = GDPlataform.socket.id;
				GDPlataform.socket.emit('conect_user',GDPlataform.myUser);
			});
			GDPlataform.socket.on('console', function (data) {
				console.log(data);
			});
		}
	},
	...
	MyChat: function(code){
		...
		this.init = function(){
			...
			//	Añadimos la funcion de añadir mensaje
			this.board.accAddMessage = function(user,message,chatCode)
			{
				this.g('dashboard-init').g('chat').accAddMessage(user,message,chatCode);
			};
			//	Genera la animación
						...
						onclick: function(){
							...
							WorkToqueder_GD.auth.onAuthStateChanged(function(user) {
								if (user)
								{
									...
									WorkToqueder_GD.myUser.domainName = window.location.hostname;
									WorkToqueder_GD.socket = io.connect('http://localhost:3000');
									WorkToqueder_GD.socket.on('connect', function (socket) {
										console.log('conectado');
										WorkToqueder_GD.socket.emit('conect_user',myUser);
									});
									WorkToqueder_GD.socket.on('console', function (data) {
										console.log(data);
									});
								}
								else
