...
*/
	...
	boardVistas.createAnimation(function(){
		...
		for(var contImagesViews=0;contImagesViews<selectViews.arImagesViews.length;contImagesViews++)
		{
			...
			if(boardVistas.qstnIsMouseHover())
			{
				var boardVistasMousePosInX = boardVistas.getMousePosInX();
				if(boardVistasMousePosInX < selectViews.espacioDeActivacionEnLados)
				{
				...
				else if(selectViews.achoTablero - selectViews.espacioDeActivacionEnLados < boardVistasMousePosInX)
				{
					...
					if(-movimientoMaximosEnXDeLasImagenes < movimientoEnXDeLasImagenes)
					{
						...
						var movDeImagenesEnX = -(velocidadEnX);
						console.info(movimientoEnXDeLasImagenes + '+' + movDeImagenesEnX + '<' + movimientoMaximosEnXDeLasImagenes);
			...
			else if(0 < selectImageNumberFramesToMoveLeft)
			{
				//	Si puede moverse mas a la izquierda
				if(movimientoEnXDeLasImagenes < 0)
				{
				...
				selectImageNumberFramesToMoveLeft--;
			}
			else if(0 < selectImageNumberFramesToMoveRight)
			{
				//	Si puede moverse mas a la derecha
				if(-movimientoMaximosEnXDeLasImagenes < movimientoEnXDeLasImagenes)
				{
				...
				selectImageNumberFramesToMoveRight--;
			}
			console.info(movimientoEnXDeLasImagenes);
